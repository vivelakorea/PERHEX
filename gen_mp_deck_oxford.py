# Self-contained Abaqus deck for the midpoint-subdivision RVE with the
# open-source OXFORD-UMAT (github.com/TarletonGroup — Demir et al.).
# Everything (mesh, per-grain materials with Bunge Euler angles, PBC
# equations, loading) lives in ONE .inp; the UMAT reads no external files
# in constants=6 mode.
#
# PBC scheme (standard 3-dummy form, cf. Tian et al. Comp. B 162 (2019)):
#   face pairs FL-F0 minus dummy jumps; dummy-3 carries the three normal
#   jumps, dummy-1/2 the shear jumps; edge/corner sets reduced to avoid
#   over-constraint. Sets are written UNSORTED in coordinate order so
#   Abaqus set-based *Equation pairing matches opposite entities.
import os
import numpy as np
from build_cube_rve import classify_and_order, verify_sets, read_orientations

OUT = "oxford_mp"
STRAIN = 0.10          # 10 % uniaxial x tension
RATE = 1e-3            # 1/s
...
grain = np.array([mapping[p - 1] for p in pid])
...
# (setA, setB, dof, dummy, ddof): u_A - u_B - u_dummy = 0
EQS = []
def block(a, b, trip):
    for dof, dum, ddof in trip:
        EQS.append((a, b, dof, dum, ddof))

# faces (interior nodes): jump semantics — dummy-3 diag, dummy-1/2 shears
block("ZL", "Z0", [(2, 1, 2), (1, 2, 1), (3, 3, 3)])
block("XL", "X0", [(3, 1, 3), (2, 2, 2), (1, 3, 1)])
block("YL", "Y0", [(1, 1, 1), (3, 2, 3), (2, 3, 2)])
# edges: chain each edge set to a reference edge of its direction
for grp, ref, trip in [
    (["X0ZL", "XLZ0", "XLZL"], "X0Z0",
     None), (["X0YL", "XLY0", "XLYL"], "X0Y0", None),
    (["Y0ZL", "YLZ0", "YLZL"], "Y0Z0", None)]:
    for s in grp:
        # jump of s relative to ref = sum of face jumps for the L-flags
        # that differ; express via the same dummy mapping per axis
        jumps = []
        for axname, axtrip in (("X", [(3, 1, 3), (2, 2, 2), (1, 3, 1)]),
                               ("Y", [(1, 1, 1), (3, 2, 3), (2, 3, 2)]),
                               ("Z", [(2, 1, 2), (1, 2, 1), (3, 3, 3)])):
            if axname + "L" in s and axname + "L" not in ref:
                jumps.append(axtrip)
        for dof in (1, 2, 3):
...
with open(f"{OUT}/rve_oxford.inp", "w") as f:
    f.write("*Heading\n** Midpoint-subdivision periodic RVE + OXFORD-UMAT\n"
    ...
    for g in grains:
        e1, e2, e3 = ori[g]
        f.write(f"*Material, name=MAT-GRAIN{g}\n*Depvar\n200,\n"
                f"*User Material, constants=6\n"
                f"{e1:.4f},{e2:.4f},{e3:.4f},{g},1,0\n")
    T = STRAIN / RATE
    f.write(f"""**
...
*End Step
""")
print(f"wrote {OUT}/rve_oxford.inp ({len(EQS)} equation groups)")
